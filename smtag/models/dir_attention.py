...
# import BasicModelOutput
from transformers.modeling_outputs import BaseModelOutput
# import _expand_mask from transformers
...
# import ACT2FN from transformers
...
class FlippableBartAttention(nn.Module):
...
class FlippableBartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model

        # self attention is flippable
        self.self_attn = FlippableBartAttention(
        ...
        # normal attention for cross-attention
...
class FlippableBartDecoder(BartPretrainedModel):
    """
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([FlippableBartDecoderLayer(config) for _ in range(config.decoder_layers)])
