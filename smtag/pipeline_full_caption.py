...
class Tagger:
    ...
    def _panelize_pipeline(self, examples: Union[List[str], str]):
        ...
        # panel_tokenizer = AutoTokenizer.from_pretrained("EMBO/sd-panelization-v2", 
        #                                 add_prefix_space=True
        #                                 )

        # panel_pipe = PanelizationPipeline(
        #     model=self.panel_model,
        #     tokenizer=panel_tokenizer,
        #     device=torch.device(0 if torch.cuda.is_available() else "cpu"),
        #     aggregation_strategy="simple"
        # )
...
class SmartTagger(Tagger):
    ...
    def __init__(
        ...
        super().__init__( 
            ...
            EXcellRobertaForTokenClassification.from_pretrained(panelizer_source),
...
#     def __init__(self, model: EXcellRobertaForTokenClassification, tokenizer: RobertaTokenizerFast, device, aggregation_strategy: str, framework: str = "pt"):
#         self.model = model
...
#     def __call__(self, inputs: Union[str, List[str]], stride: int = 100):
#         self.stride = stride
#         all_outputs = []
#         for model_inputs in self.preprocess(inputs, offset_mapping=None, stride=self.stride):
#             model_outputs = self._forward(model_inputs)
#             all_outputs.append(model_outputs)

#         pre_tokenized_inputs, offset_mapping_original = self._pretokenize(inputs)
#         model_inputs = self.tokenizer(
...
#             ignore_labels=None,
#             stride=self.stride, 
#             aggregated_tokenizer=model_inputs,
...
#         pre_tokenized_inputs, offset_mappings = self._pretokenize(sentence)
...
#         if offset_mappings:
#             model_inputs["offset_mapping"] = offset_mappings
                    
#         model_inputs["sentence"] = sentence

#         print("Length of the input in tokens",  len(model_inputs["input_ids"]))
...
#         while first_token < len(idx_lookup):
#             start = max(0,first_token-stride)
#             end = min(start + self.model.config.max_length - 4, len(idx_lookup))
#             print(start, end)
...
#     def _pretokenize(self, sentence: str) -> Tuple[List[int], List[Tuple[int,int]]]:
#         word = ''
#         start = 0
#         pre_tokenized_inputs, offset_mappings = [], []
...
#             elif char == " ":
#                 if word not in [""]:
#                     offset_mappings.append((start, i))
#                     pre_tokenized_inputs.append(word)
#                 word = ''
#             else:
#                 if word not in [""]:
#                     offset_mappings.append((start, i))
...
#                 pre_tokenized_inputs.append(char)
#                 offset_mappings.append((i, i+1))
#                 word = ''

#         assert len(pre_tokenized_inputs) == len(offset_mappings)
#         return pre_tokenized_inputs, offset_mappings
...
#                     ignore_labels=None, 
#                     stride=0,
#                     aggregated_tokenizer=None,
#                     offset_mapping_original=None):
#         sentence = model_outputs[0]["sentence"]
#         # aggregated_tokenizer_outputs = self.tokenizer(sentence,
#         #     return_tensors=self.framework,
#         #     return_special_tokens_mask=True,
...
#         input_ids = aggregated_tokenizer["input_ids"]
#         offset_mapping = aggregated_tokenizer["offset_mapping"]
#         special_tokens_mask = aggregated_tokenizer["special_tokens_mask"]
#         print(offset_mapping_original)
...
#             print(offset)
#             if offset == torch.tensor([0, 0]):
#                 new_offset_mapping.append(torch.tensor([0, 0]))
#             else:
#                 if offset[0] == torch.tensor(0):
#                     start = offset_mapping_original[original_index][0]
#                     end = offset_mapping_original[original_index][0] + offset[1]
#                     if end == offset_mapping_original[original_index][1]:
#                         new_offset_mapping.append(torch.tensor([start, end]))
#                         original_index += 1
#                 else:
#                     start = offset_mapping_original[original_index][0] + offset[0]
#                     end = start + offset[1]
#                     if end == offset_mapping_original[original_index][1]:
#                         new_offset_mapping.append(torch.tensor([start, end]))
#                         original_index += 1

#         print(len(new_offset_mapping))
